...
def assemble_policy_fingerprint(
    ...
    """Fingerprint = ``size`` NLLs followed by ``size`` entropies.

    Pads with 0.0 (and truncates) so the result is exactly ``2 * size`` long,
    matching the frozen ``policy_fingerprint`` width.
    """
...
def compute_policy_fingerprint(
    ...
    *,
    ...
    """Fingerprint (per-prompt NLL + entropy) on the fixed MATH prompts."""
    import torch
