...
class ChainSplitTests(_SynthCase):
    def test_two_directions(self):
        ...
        self.assertEqual(splits[0].train_chain, "1")
        self.assertEqual(splits[0].test_chain, "0")
        self.assertEqual(splits[1].train_chain, "0")
        self.assertEqual(splits[1].test_chain, "1")
    ...
    def test_train_test_chain_disjoint(self):
        for s in chain_splits(self.data):
            ...
            self.assertEqual(set(s.train["chain_id"]), set(s.train_chains))
    ...
    def test_leave_one_chain_out_supports_three_chains(self):
        with tempfile.TemporaryDirectory() as tmp:
            generate(tmp, labels=192, chains=3, candidates_per_state=8, seed=1729)
            data = load_parquets(tmp)
            ...
            for s in splits:
                self.assertEqual(set(s.test["chain_id"]), {s.test_chain})
                self.assertEqual(set(s.train["chain_id"]), set(s.train_chains))
                self.assertNotIn(s.test_chain, s.train_chains)
...
class FeaturizeTests(_SynthCase):
    ...
    def test_tap_block_shapes(self):
        ...
        self.assertEqual(b.fingerprint.shape, (n, 32))
        self.assertEqual(b.history.shape, (n, 8, F.HISTORY_RECORD_DIM))
        self.assertEqual(b.history_mask.shape, (n, 8))
