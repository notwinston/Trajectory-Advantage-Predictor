...
class SchemaContractTests(unittest.TestCase):
    ...
    def test_frozen_vector_widths(self):
        self.assertEqual(schema.POLICY_FINGERPRINT_DIM, 32)
        self.assertEqual(schema.CANDIDATE_EMBEDDING_DIM, 256)
        self.assertEqual(schema.GRADIENT_SKETCH_DIM, 64)
        self.assertEqual(schema.TRAJECTORY_EMBEDDING_DIM, 256)
...
class SynthGenerationTests(unittest.TestCase):
        ...
        for labels, (states_per_chain, cps) in layouts.items():
            with self.subTest(labels=labels):
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    # history: per chain sum_{i} min(i, 8) applied updates.
                    per_chain_history = sum(min(i, 8) for i in range(states_per_chain))
                    self.assertEqual(counts["history.parquet"], 2 * per_chain_history)

    def test_design_scale_is_schema_valid(self):
        with tempfile.TemporaryDirectory() as tmp:
            counts = generate(tmp, labels=192, chains=3, candidates_per_state=8, seed=1729)
            schema.validate_parquet_dir(tmp)
            self.assertEqual(counts["states.parquet"], 3 * 8)
            self.assertEqual(counts["candidates.parquet"], 192)
    ...
    def test_vector_widths_in_data(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(all(len(r["policy_fingerprint"]) == 32 for r in states))
            ...
            self.assertTrue(all(len(r["trajectory_embedding"]) == 256 for r in trajectories))
