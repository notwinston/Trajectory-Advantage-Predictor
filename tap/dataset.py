...
* :func:`load_parquets`     -> :class:`TapData` (raw frames + a candidate frame
  ...
  utility contract).
* :func:`chain_splits`      -> leave-one-chain-out directions.
  Every candidate of a state
...
HISTORY_WINDOW = 8  # latest eight applied update batches (spec)
...
def chain_splits(data: TapData) -> List[ChainSplit]:
    """Return leave-one-chain-out directions (never a random split).

    With >=128 labels, the last two states (by step) of one training chain are
    moved into ``val`` for early stopping; ``fit`` is the remainder.
    """
    joined = data.joined
    chains = data.chains
    if len(chains) < 2:
        raise ValueError(f"chain_splits expects at least 2 chains, got {chains}")
    use_early_stop = data.n_labels >= EARLY_STOP_MIN_LABELS

    splits: List[ChainSplit] = []
    for direction, test_chain in enumerate(chains):
        train_chains = tuple(chain for chain in chains if chain != test_chain)
        train = joined[joined["chain_id"].isin(train_chains)].reset_index(drop=True)
        test = joined[joined["chain_id"] == test_chain].reset_index(drop=True)
        if use_early_stop:
            validation_chain = train_chains[-1]
            val_ids = _last_two_state_ids(train[train["chain_id"] == validation_chain])
