...
* :class:`tap.model.SmallTAP` uses :func:`build_tap_blocks`, the structured
  ...
  state numerics, policy fingerprint, and a padded ``[N, 4, rec_dim]`` history
...
class TapBlocks:
    ...
    fingerprint: np.ndarray  # [N, 16]
    history: np.ndarray  # [N, 4, HISTORY_RECORD_DIM]
    history_mask: np.ndarray  # [N, 4] bool (True = real record)
    history_rel_age: np.ndarray  # [N, 4] int (0 padding, else 1..4)
