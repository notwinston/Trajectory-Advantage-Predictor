...
features fitting on the TRAIN candidates only, then a scikit-learn regressor
predicts ``utility_points`` (used purely as a within-state ranking signal).
...
Pure CPU: numpy/pandas/scikit-learn. No torch.
"""
...
def _gbt_factory():
    ...
    # Small/shallow: ~72 noisy labels. Shallow stumps + low LR resist overfit.
