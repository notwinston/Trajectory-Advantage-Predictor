...
* Joining ``candidates`` -> ``states`` on ``state_id``::
      ...
      utility_points == 1000 * (
          0.8 * (matched_probe_nll_before  - matched_probe_nll_after)
        + 0.2 * (global_probe_nll_before   - global_probe_nll_after)
        - 0.03 * max(generic_kl_after - generic_kl_before, 0)
      )
...
HISTORY_WINDOW = 4  # latest four applied update batches
        ...
        for state_index in range(states_per_chain):
            ...
            for k in range(cps):
                ...
                utility_points = 1000.0 * (
                    0.8 * matched_gain
                    + 0.2 * global_gain
                    - 0.03 * max(incremental_generic_kl, 0.0)
                )
...
def main(argv: List[str] | None = None) -> int:
    ...
    parser.add_argument("--labels", type=int, default=72, help="total candidate labels (48/72/128)")
