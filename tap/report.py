...
engine name is recorded inside ``report.md``. reportlab (pure-Python) is the
guaranteed fallback so the pipeline always produces a >1KB PDF on CPU-only ARM64
with no system TeX/pandoc installed.

Pure CPU. reportlab is imported lazily inside the fallback.
"""
...
def render_markdown(results: pd.DataFrame, analysis: Dict, pdf_engine: str) -> str:
    ...
    lines.append(
        ...
        f"2 chains). Evaluation: leave-one-chain-out, averaged over both directions. "
...
def render_latex(results: pd.DataFrame, analysis: Dict) -> str:
    ...
    return "\n".join(
        [
            ...
            f"({analysis['n_labels']} candidate labels, {analysis['n_states']} states, 2 chains). "
            f"Leave-one-chain-out, averaged over both directions. Seed {analysis['seed']}.",
            "",
            param_line,
            "",
...
def _reportlab_pdf(pdf_path: Path, results: pd.DataFrame, analysis: Dict) -> bool:
    ...
    story.append(
        Paragraph(
            ...
            f"{analysis['n_states']} states, 2 chains. Leave-one-chain-out, both directions "
...
def write_pdf(out_dir: Path, tex_path: Path, md_text_for: Dict, results: pd.DataFrame, analysis: Dict) -> str:
    ...
    if _try_pdfkit(html, pdf_path):
        return "pdfkit"
    _reportlab_pdf(pdf_path, results, analysis)
    return "reportlab"
